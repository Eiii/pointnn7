...
class _WeatherBase(Network):
    def forward(self, hist, mask, times, station_meta, station_pos,
        ...
        encoded_hist = self.encode(hist, mask, times, station_meta,
                                   station_pos, station_idxs, allowed)
        pred = self.predict(encoded_hist, mask, times, station_idxs,
                            station_pos, target_pos, target_allowed)
        return pred


    def encode(self, hist, mask, times, station_meta, station_pos,
               station_idxs, allowed):
        ...
        space_in = torch.cat([hist, hist_meta], dim=2)
        for space, time, combine in zip(self.space, self.time, self.combine):
            space_neigh = space(hist_pts, hist_pts, space_in, space_dist_fn)
            time_in = torch.cat([space_in, space_neigh], dim=2)
            time_neigh = time(times, times, time_in, time_dist_fn)
            combine_in = torch.cat([space_in, space_neigh, time_neigh], dim=2)
            combine_out = combine(combine_in)
            space_in = combine_out
        return combine_out
    ...
    def predict(self, encoded_hist, mask, times, station_idxs, station_pos,
                target_pos, allowed):
        hist_pts = self._get_by_idx(station_idxs, station_pos)
        target_dist_fn = partial(target_dist, mask, times, allowed)
        tgt_latent = self.target_xf(target_pos, hist_pts, encoded_hist,
                                    target_dist_fn)
...
class WeatherTPC(_WeatherBase):
    ...
    def make_encoders(self, feat_size, weight_hidden, c_mid, final_hidden,
        ...
        args = {'weight_hidden': weight_hidden, 'c_mid': c_mid,
                'final_hidden': final_hidden,
                'neighbors': neighbors, 'c_in': ls, 'dim': 3,
                'c_out': target_size}
        self.target_xf = pointconv.PointConv(**args)
